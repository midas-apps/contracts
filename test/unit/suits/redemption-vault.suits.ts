  ...
  describe(rvName, function () {
    ...
    describe('common', () => {
        ...
        describe('loan lp', () => {
          ...
          describe('preferLoanLiquidity=true', () => {
            ...
            it('when lp mToken balance conversion is not divisible by tokenOutRate', async () => {
              ...
              // Nested-floor capacity of 100e18 is 16...666. A ceiled capacity
              // would invert to 100e18 + 2 and revert on transferFrom.
              // User amount is larger than LP capacity so gross is not capped
              // to missingAmount.
            ...
            it('when nested tokenOut conversion would be 1 wei short with a combined ceil', async () => {
              const {
                owner,
                redemptionVault,
                stableCoins,
              ...
              // Combined ceil(missing * 0.9 / 0.9) = missing, but inner
              // floor/floor pays missing - 1. Two-step inverse sizes +1 mToken.
              ...
              await mTBILL
                .connect(owner)
                .approve(redemptionVault.address, amountMTokenIn);
              await mTokenLoan.mint(loanLp.address, parseUnits('1000'));
              await mTokenLoan
                .connect(loanLp)
                .approve(redemptionVault.address, parseUnits('1000'));
              await mintToken(
                stableCoins.usdt,
                redemptionVaultLoanSwapper,
                1000,
              ...
              const lpMTokenBefore = await mTokenLoan.balanceOf(loanLp.address);

              await expect(
                redemptionVault['redeemInstant(address,uint256,uint256)'](
              ...
              expect(await mTokenLoan.balanceOf(loanLp.address)).eq(
                lpMTokenBefore.sub(mTokenAToPull),
              );
              expect(mTokenAToPull).eq(BigNumber.from('111111111111111111112'));
              expect(
                mTokenAmountToTokenOutAmount(
                  BigNumber.from('111111111111111111111'),
                  parseUnits('0.9'),
                  parseUnits('0.9'),
                ),
      ...
      describe('setRequestRedeemer()', () => {
        ...
        it('should fail: if redeemer is zero address', async () => {
          const { redemptionVault, owner } = await loadRvFixture();
      ...
      describe('setLoanApr()', () => {
        ...
        it('should fail: when function is paused', async () => {
          ...
          await pauseVaultFn(
            { pauseManager, owner },
            redemptionVault,
            encodeFnSelector('setLoanApr(uint256)'),
          );
        ...
        it('succeeds with scoped permission and vault admin role', async () => {
          ...
          await setLoanAprTest({ redemptionVault, owner }, 7500, {
            from: regularAccounts[0],
          });
        });
      });
      ...
      describe('redeemRequest()', () => {
        ...
        it('should fail: if exceed tokenOut allowance', async () => {
          const {
            owner,
            redemptionVault,
            stableCoins,
        ...
        it('should decrease tokenOut allowance if allowance < UINT_MAX', async () => {
          const {
            owner,
            redemptionVault,
            stableCoins,
        ...
        it('should not decrease tokenOut allowance if allowance = UINT_MAX', async () => {
          const {
            owner,
            redemptionVault,
            stableCoins,
        ...
        it('should fail: when a second request would exceed remaining tokenOut allowance', async () => {
          const {
            owner,
            redemptionVault,
            stableCoins,
        ...
        it('should decrease tokenOut allowance for instant and request portions', async () => {
          const {
            owner,
            redemptionVault,
            stableCoins,
      ...
      describe('approveRequest()', async () => {
        ...
        describe('avgRate=false', async () => {
          ...
          it('should not decrease tokenOut allowance on approve', async () => {
            const {
              owner,
              mockedAggregator,
              mockedAggregatorMToken,
            ...
            const tokenConfigBeforeApprove = await redemptionVault.tokensConfig(
              stableCoins.dai.address,
            );
            ...
            const tokenConfigAfterApprove = await redemptionVault.tokensConfig(
              stableCoins.dai.address,
            );

            expect(tokenConfigAfterApprove.allowance).eq(
              tokenConfigBeforeApprove.allowance,
            );
            expect(tokenConfigAfterApprove.allowance).eq(parseUnits('900'));
          });

          it('should approve when remaining tokenOut allowance is zero after request', async () => {
            const {
              owner,
              mockedAggregator,
              mockedAggregatorMToken,
            ...
            expect(
              (await redemptionVault.tokensConfig(stableCoins.dai.address))
                .allowance,
            ).eq(0);
      ...
      describe('rejectRequest()', async () => {
        ...
        it('should not restore tokenOut allowance on reject', async () => {
          const {
            owner,
            mockedAggregator,
            mockedAggregatorMToken,
          ...
          const tokenConfigBeforeReject = await redemptionVault.tokensConfig(
            stableCoins.dai.address,
          );
          expect(tokenConfigBeforeReject.allowance).eq(parseUnits('900'));
          ...
          const tokenConfigAfterReject = await redemptionVault.tokensConfig(
            stableCoins.dai.address,
          );
          expect(tokenConfigAfterReject.allowance).eq(
            tokenConfigBeforeReject.allowance,
          );
        });
      ...
      describe('_obtainLoanLpLiquidityExternal', () => {
        ...
        it('sizes mToken with nested ceil inverse so inner floor/floor is not 1 wei short', async () => {
          const {
            owner,
            redemptionVault,
            stableCoins,
          ...
          expect(await stableCoins.usdt.balanceOf(redemptionVault.address)).eq(
            missingTokenOut,
          );
          expect(await mTokenLoan.balanceOf(loanLp.address)).eq(
            lpMTokenBefore.sub(mTokenAToPull),
          );
          expect(mTokenAToPull.gt(missingTokenOut)).eq(true);
        });

        it('does not pull more mToken than LP balance when capacity is not divisible', async () => {
          const {
            owner,
            redemptionVault,
            stableCoins,
          ...
          const lpBalance = parseUnits('100');
          await mTokenLoan.mint(loanLp.address, lpBalance);
          await mTokenLoan
            .connect(loanLp)
            .approve(redemptionVault.address, lpBalance);
          await mintToken(stableCoins.usdt, redemptionVaultLoanSwapper, 1000);
          ...
          const lpMTokenAfter = await mTokenLoan.balanceOf(loanLp.address);
          const pulled = lpBalance.sub(lpMTokenAfter);
          expect(pulled).lte(lpBalance);
          expect(pulled).eq(
            tokenOutAmountToMTokenAmount(
              mTokenAmountToTokenOutAmount(
                lpBalance,
      ...
      describe('_calculateHoldbackPartRateFromAvg', () => {
        ...
        it('returns 0 when amountMTokenInstant is 0', async () => {
          const { redemptionVault } = await loadRvFixture();
          const amountMToken = parseUnits('100');
