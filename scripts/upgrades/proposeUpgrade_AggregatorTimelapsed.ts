...
const func: DeployFunction = async (hre: HardhatRuntimeEnvironment) => {
  ...
  for (const mToken of mTokens) {
    hre.mtoken = mToken;
    try {
      const outcome = await proposeAggregatorTimelapsedForMToken(hre, mToken);
      if (outcome === 'skipped') {
        skipped++;
      } else {
        upgraded++;
...
// Single product:
// yarn hardhat runscript scripts/upgrades/proposeUpgrade_AggregatorTimelapsed.ts --network <NETWORK> --mtoken <MTOKEN>
//
// Batch (targets in scripts/upgrades/configs/aggregator-timelapsed-config.ts):
// yarn hardhat runscript scripts/upgrades/proposeUpgrade_AggregatorTimelapsed.ts --network <NETWORK> --action <UPGRADE_ID>
