...
contract LzEndpointV2Mock is ILayerZeroEndpointV2, MessagingContext {
    ...
    uint256 public nextComposerMsgValue;
        ...
        returns (MessagingReceipt memory receipt)
    {
        ...
        address lzEndpoint = address(
            lzEndpointLookup[_params.receiver.bytes32ToAddress()]
        );
        require(
        ...
        LzEndpointV2Mock(payable(lzEndpoint)).receivePayload{value: dstAmount}(
            origin,
            packet.receiver.bytes32ToAddress(),
            payloadHash,
            receiveMessage,
    ...
    function getExecutorFee(uint256 _payloadSize, bytes calldata _options)
        public
        view
        returns (uint256)
    {
    ...
    function setReadResponse(address destAddr, bytes memory resolvedPayload)
        external
    {
    ...
    function splitOptions(bytes calldata _options)
        internal
        pure
    ...
    function setNextComposerMsgValue() external payable {
        nextComposerMsgValue = msg.value;
    }
    ...
    function sendCompose(
        ...
        ILayerZeroComposer(to).lzCompose{value: nextComposerMsgValue}(
            msg.sender,
            guid,
            message,
            address(0),
